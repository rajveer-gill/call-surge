    ...
    *,
    ...
    if str(business_info.get("booking_mode") or "").strip().lower() == "external":
        ...
        help_lines.append(
            ...
            "  • SAY SO OUT LOUD, EARLY. The FIRST time you start taking day/time details, "
            "tell the caller plainly that this is a REQUEST and the salon will confirm it — "
            "e.g. \"Just so you know, I'm putting in a request for you rather than booking "
            "it; the salon will confirm the time with you.\" Say it once, in plain words, "
            "before you ask for the day. A caller who thinks they have an appointment when "
            "they only have a request will turn up to a chair that isn't theirs.\n"
            "  • Say REQUEST every time you refer to it — \"your request\", \"put the request "
            "in\" — never \"your appointment\" or \"your booking\".\n"
    ...
    if staff:
        ...
        if restricted_lines:
            ...
            staff_block += (
                ...
                + "\n".join(all_lines)
                + "\n  CRITICAL — before you agree to OR refuse any day/time, work out which weekday the "
                "caller's requested date falls on and LOOK IT UP in THAT stylist's own line above. The "
                "answer decides which of exactly two things you do:\n"
                "  (a) The weekday IS listed for that stylist (and the time is inside their hours, and the "
                "date is not one of their OFF dates) — they are AVAILABLE. Continue with the booking and say "
                "NOTHING about their schedule. Do not raise availability, do not list their other days, do "
                "not apologise. This is the common case.\n"
                "  (b) The weekday is NOT listed for that stylist, or the time is outside their hours that "
                "day, or the date is one of their OFF dates — they are unavailable. Do NOT book them and do "
                "NOT say they are booked, all set, scheduled, or confirmed. Tell the caller that stylist "
                "isn't available then, name the days/times that stylist DOES work, and offer one of those or "
                "another stylist.\n"
                "  NEVER tell a caller a stylist does not work on a day that IS listed on that stylist's "
                "line. If you are about to say \"doesn't work on <day>\", re-read their line first: if <day> "
                "appears there, you are wrong — they are available and you must proceed under (a). Saying a "
                "stylist is unavailable on a day they actually work turns away a real customer, which is "
                "worse than any other mistake you can make here.\n"
                "  Worked examples. Stylist works Thursday, Friday, Sunday; caller asks for Friday: Friday IS "
                "on their line, so this is case (a) — book it and never mention their schedule. Same stylist, "
                "caller asks for Tuesday: Tuesday is NOT on their line, so this is case (b) — say they don't "
                "work Tuesdays and offer Thursday, Friday, or Sunday. Stylist whose line says 'works any day "
                "the shop is open': every open day is case (a) — book the requested day and never restrict "
                "them to another stylist's days."
            )
    ...
    if include_booked_slots:
        ...
        if has_configured_services:
            service_booking_rules = (
                ...
                "A caller can ask for MORE THAN ONE service in one visit (\"a haircut and an "
                "all over color\", \"can you add a highlight to that\"). Take all of them: put "
                "EVERY service they asked for in the reason field, joined with ' + ' (e.g. "
                "\"Haircut + All Over Color\"). When they add a service to a request you have "
                "already taken, output a NEW BOOKING line on that same turn listing BOTH the "
                "original service and the new one — saying you have added it without emitting "
                "that line adds nothing, and the salon only ever sees the line. "
...
{service_booking_rules}reply with EXACTLY: BOOKING: name|phone|email|date|time|reason|staff (| separator). Field 1 name is the CALLER's name (the customer)—NEVER a stylist. Field 7 staff is ONLY for the stylist when they chose one. The reason field holds the service name when a service menu exists, or a short visit note otherwise. RULES: (1) You MUST include the caller's name in field 1—if they haven't given it, ask for their name first, then output BOOKING. Never put a stylist name in field 1. (2) For phone and email: leave empty (we have phone from the call; we do not collect email). (3) Date must be YYYY-MM-DD. Today is {today_dow} {today_str}; tomorrow is {tomorrow_dow} {tomorrow_str}. Use the DATE REFERENCE list to map any weekday the caller names to its date—do NOT compute the day of week yourself (e.g. "tomorrow" = {tomorrow_str}). (4) Time: write it WITH its am/pm period (e.g. "1 PM", "9:30 AM", "12 PM" for noon)—do NOT convert to 24-hour/military time yourself. Callers say "three o'clock", "at three" or "3:00" far more often than "3 PM", and they mean the reading inside our opening hours—NEVER 3 AM. Do NOT ask "AM or PM?" for such a time: take the in-hours reading, write it with the period you inferred, and simply say it back as you confirm ("three PM, got it") so they can correct you. Only ask which half of the day they mean when BOTH readings are inside our opening hours. (5) Do not output BOOKING until you have at least name, date, and time. (6) {confirm_rule} (7) When multiple stylists and a service menu exist, ask which SERVICE they want FIRST; then suggest only the stylists who provide that service and ask which they prefer (or anyone is fine)—do not ask for the stylist before the service. (8) Be proactive: never end a turn with vague filler like "let me get the rest of your details", "one moment", or "let me pull that up" and then stop. While any detail is still missing, ALWAYS end your reply by directly asking the caller for the single next missing item (their name, the day/time, the stylist, or the service) so they know exactly what to say—do not make them ask what you need. (9) If the caller changes any already-agreed detail (service, stylist, date, or time) — or ADDS another service — after you have output BOOKING, you MUST output a NEW BOOKING line on that same turn with ALL fields updated—carrying forward every field they did NOT change, and listing every service they now want, joined with ' + '. Do not just acknowledge it in words, or it will NOT be saved. This applies to adding a second service exactly as it does to a time change. (10) NEVER ask again for something the caller has already told you on this call. If the day or time they wanted does not work, you keep the service, the stylist and their name, and ask ONLY for a different day or time—changing the day does not restart the booking. If you have already asked which stylist they want twice, stop asking: take the request without a stylist and tell them the salon will assign one. Asking a caller to repeat what they just said is the fastest way to lose them."""
